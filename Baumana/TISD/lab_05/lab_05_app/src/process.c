        ...
        if (time_until_work <= 0)
        {
            ...
            else
            {
                ...
                if (log1.tasks_out % 100 == 0)
                {
                    ...
                    if (log1.tasks_failed > 0)
                    {
                        printf("Ошибка : переполнения очереди -> количество неудавшихся запросов %d\n", log1.tasks_failed);
                        // break;
...
void simulate_list()
{
    ...
    while (log1.tasks_out != 1000)
    {
        if (time_until_fetch1 <= 0)
        {
            time_until_fetch1 = random_float(q1_f_s, q1_f_f);
            task_t *task = create_task();
            int rc = push_in_list_queue(queue1, task);
            if (rc == OVERFLOW_ERROR)
            {
                log1.tasks_failed += 1;
                delete_task(task);
        ...
        if (time_until_fetch2 <= 0)
        {
            time_until_fetch2 = random_float(q2_f_s, q2_f_f);
            task_t *task = create_task();
            int rc = push_in_list_queue(queue2, task);
            if (rc == OVERFLOW_ERROR)
            {
                log2.tasks_failed += 1;
                delete_task(task);
    ...
    while (1)
    {
        ...
        switch (choice)
        {
            ...
            case 2:
            {
                task_t *task = create_task();
                clock_t start = clock();
                rc = push_in_list_queue(list_queue, task);
                double end = (double) (clock() - start) / CLOCKS_PER_SEC;
                if (rc != 0)
                {
                    if (rc == OVERFLOW_ERROR)
