...
void insertion_sort(size_t n, struct items_t *arr)                             
{
...
int lowdens_to_highdens_sort(char *filename)
{
    ...
    //     printf("%s\t\n%lf\t\n%lf\t\n", items[b].name, items[b].mass,items[b].amount);
    // }
    // printf("%s\t\n%lf\t\n%lf\t\n", items[0].name, items[0].mass,items[0].amount);
    printf("\n");
...
void show_err_message(int rc)
{
    ...
    switch(rc)
    {
        ...
        case ERR_DATA:
            fprintf(stdout, "Невалидные данные предмета\n");
            break;
