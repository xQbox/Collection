...
def NewtonSystem(jacobianFunc, funcs : list, x0 : list, maxIter=10, epsilon=1e-8):
    def f(x):
    ...
    n = 1
    while True:
        # решаем слау методом гаусса для нахождения приращений
        dx = Gauss(jacobianFunc(*xk), [-y for y in f(xk)])
        # xi(s+1) = xi(s) + dxi(s), получаем следующие точки
        xnext = [xk[i] + dx[i] for i in range(len(xk))]
        
        # если сумма квадратов отклонений меньше эпсилон, или закончились итерации -> закончить итерации
...
# Метод нахождения корня (метод половинного деления)
def HalfDivision(f, y, a, b, maxIter=10, epsilon=1e-8):
    n = 1
    while True:
        c = (a + b) / 2
